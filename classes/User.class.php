...
class User {
    ...
    public function save() {
        if (!$this->user_id) {
            ...
            $stmt->execute(array($this->user_id,$this->username,$this->vorname,$this->nachname,$this->email,$this->perm,$this->locked,$this->salutation));
            $stmt = DBManager::get()->prepare("INSERT INTO users_info (user_id, url, workplace) VALUES (?,?,?)");
            $stmt->execute(array($this->user_id, $this->url, $this->workplace));
        } else {
            $stmt = DBManager::get()->prepare("UPDATE users SET username=?, vorname=?, nachname=?, email=?, perm=?, locked=?, salutation=? WHERE user_id=?");
            $stmt->execute(array($this->username,$this->vorname,$this->nachname,$this->email,$this->perm,$this->locked,$this->salutation, $this->user_id));
