...
class Search
{
    ...
    public static function searchSimple($text, $category_id = FALSE)
    {
        ...
        {
            $rr = $db->query(sprintf("SELECT p.* FROM plugins p, categories_plugins cp WHERE p.plugin_id IN (SELECT ta.object_id FROM tags t, tags_objects ta WHERE LOWER(t.tag) LIKE LOWER('%s')) AND cp.plugin_id=p.plugin_id AND cp.category_id='%s' AND p.approved=1 ORDER BY p.name",'%'.$txt.'%',$catagory_id))->fetchAll();
