...
class Screenshot {
    ...
    public function save() {
        if (!$this->screenshot_id) {
            ...
            $stmt->execute(array($this->screenshot_id, $this->plugin_id, $this->mkdate, $this->title_screen, $this->file_id, $this->titel));
        } else {
            $stmt =  DBManager::get()->prepare("UPDATE screenshots SET title_screen=?, titel=? WHERE screenshot_id=?");
            $stmt->execute(array($this->title_screen, $this->titel, $this->screenshot_id));
        }
    }
